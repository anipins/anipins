...
export default function Toaster() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-24 left-1/2 z-[120] flex w-[min(92vw,420px)] -translate-x-1/2 flex-col items-center gap-2 md:bottom-6">
      <AnimatePresence>
        {items.map(t => (
          <motion.div key={t.id} initial={{ opacity: 0, y: 16, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }}
            ...
            role="status" className={`glass hairline w-fit rounded-2xl px-5 py-3 text-sm shadow-2xl ${t.kind === "ok" ? "border-gold/30 text-paper" : "border-red-500/40 text-red-300"}`}>
