...
export default function CollectionPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <section className="w-full px-3 sm:px-4 md:px-6 xl:px-8 pt-28 md:pt-32">
      <Link href="/saves" className="text-sm text-fog hover:text-paper">← Collections</Link>
      <div className="mt-2 flex items-center gap-3"><h1 className="font-display text-3xl font-semibold md:text-4xl">{data?.collection?.name || "…"}</h1>{data && !data.collection.is_private && <button onClick={()=>setShare(true)} className="btn-ghost !px-4 !py-2">Share board</button>}</div>
      ...
      <div className="masonry">
        {data?.items?.map((a: any, i: number) => (
          <div key={a.id} className="relative group/wrap">
            ...
            {data.owner && <button onClick={() => remove(a.id)} title="Remove from collection"
      ...
      {share && <ShareMenu url={location.href} title={data?.collection?.name || "AniPins board"} onClose={()=>setShare(false)}/>}
    </section>
  );
}
