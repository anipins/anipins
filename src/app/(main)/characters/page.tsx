    ...
    <div className="min-h-screen bg-[#080808]">
      ...
      <main className="pt-16 pb-20 md:pb-8">
        <div className="max-w-[1800px] mx-auto px-4 py-8">
          ...
          <p className="text-gray-500 mb-8">Every character with artwork on AniPins.</p>

          {/* Controls */}
          <div className="flex flex-wrap gap-3 mb-8 items-center">
            <div className="relative flex-1 max-w-sm">
              <input type="text" placeholder="Search characters..." value={search}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {characters.map((char: any, i: number) => (
                <Link key={char.id} href={`/characters/${char.slug}`}
                  className="group animate-fadeInUp" style={{ animationDelay: `${Math.min(i * 30, 400)}ms` }}>
                  <div className="relative aspect-[3/4] rounded-xl overflow-hidden mb-2 bg-[#111]">
                    <img src={proxyUrl(char.thumbnail_url)} alt={char.name}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                  <p className="text-sm font-medium truncate">{char.name}</p>
                  <p className="text-xs text-gray-500 truncate">{char.anime_name} · {char.artwork_count} artwork{char.artwork_count !== 1 ? 's' : ''}</p>
                </Link>
              ))}
            </div>
          ) : (
