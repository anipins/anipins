...
export type SessionUser = { id: number; email: string; name: string; nickname: string; avatar: string; cover: string; bio: string; is_public: number; role: string };
...
export async function getUser(): Promise<SessionUser | null> {
  try {
    ...
    const u = await row(
      "SELECT u.id, u.email, u.name, u.nickname, u.avatar, u.cover, u.bio, u.is_public, u.role FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ? AND s.expires_at > ?",
      token, Date.now()
    );
