...
public class LoginActivity extends Activity {
    private ApiClient api; private boolean register; private String challenge=""; private TextView heading; private EditText name,email,password; private Button submit,toggle;
    ...
    private void submit(){String mail=email.getText().toString().trim(),secret=password.getText().toString();if(challenge.isEmpty()&&(!mail.contains("@")||secret.length()<6)){Toast.makeText(this,"Enter a valid email and password of at least 6 characters",Toast.LENGTH_LONG).show();return;}if(!challenge.isEmpty()&&secret.trim().length()<6){Toast.makeText(this,"Enter your authentication or recovery code",Toast.LENGTH_LONG).show();return;}submit.setEnabled(false);submit.setText(challenge.isEmpty()?(register?"Creating…":"Signing in…"):"Verifying…");try{JSONObject body=challenge.isEmpty()?new JSONObject().put("email",mail).put("password",secret):new JSONObject().put("challenge",challenge).put("code",secret.trim());if(register&&challenge.isEmpty())body.put("name",name.getText().toString().trim());api.post(register?"/api/auth/register":"/api/auth/login",body,(status,data,error)->{submit.setEnabled(true);if(status==200&&data.optBoolean("requiresTwoFactor")){challenge=data.optString("challenge");heading.setText("Security verification");name.setVisibility(android.view.View.GONE);email.setVisibility(android.view.View.GONE);password.setText("");password.setHint("6-digit code or recovery code");password.setInputType(InputType.TYPE_CLASS_TEXT);submit.setText("Verify and sign in");toggle.setVisibility(android.view.View.GONE);return;}submit.setText(challenge.isEmpty()?(register?"Create account":"Sign in"):"Verify and sign in");if(status==200){Toast.makeText(this,"Signed in to AniPins",Toast.LENGTH_SHORT).show();finish();}else Toast.makeText(this,data.optString("error","Could not sign in. Check your connection."),Toast.LENGTH_LONG).show();});}catch(JSONException ignored){submit.setEnabled(true);}}
}
